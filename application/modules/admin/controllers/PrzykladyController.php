...
class Admin_PrzykladyController extends kCMS_Admin
{
		
		public function preDispatch() {
			$this->view->controlname = "Przykładowe mieszkania";
			
			$this->duzaszerokosc = '610';
			$this->duzawysokosc = '540';
			$this->sql_table = 'przykladowe';
			$this->backto = '/admin/przyklady/';
		}
				...
				if ($this->_request->getPost()) {
					...
					if ($form->isValid($formData)) {
						...
						if ($obrazek) {
							...
							move_uploaded_file($_FILES['obrazek']['tmp_name'], FILES_PATH.'/przykladowe/'.$plik);
							$upfile = FILES_PATH.'/przykladowe/'.$plik;
							chmod($upfile, 0755);
					...
					$plik = zmiana($formData['nazwa']).'.'.zmiennazwe($obrazek);
						...
						if ($form->isValid($formData)) {
							...
							if ($obrazek) {
								...
								unlink(FILES_PATH."/przykladowe/".$wpis->plik);
